...
`lib` is a component of `Jumpscale` (see [here](https://github.com/Jumpscale/core)). It extends `Jumpscale` functionalities by adding additional automation tools. It is the home for non core `Jumpscale` clients(e.g., Kubernetes, postgresql, rivine), tools(e.g., raml), system abstraction layers(e.g., ubuntu, docker) and different data format interactions(e.g., capnp).

See [docs](docs/README.md) for more details.

## Installtion

To use `lib` `Jumpscale` [core](https://github.com/Jumpscale/core) needs to be installed, since `lib` depends on it.
Installing `Jumpscale` using [bash](https://github.com/Jumpscale/bash) should provide the user with an installation that includes `lib`.

Follow the instructions to install and setup `bash` repo and then run the following command to have `lib` installation with all the necessary dependencies:

```bash
export JUMPSCALEBRANCH={BRANCH}
ZInstall_host_js_full
```
...
Start the `Jumpscale` shell by typing `jumpscale` into the console.
...
This instance needs to be configured using config manager before it can be used, which will make it easier to reuse the same instance and ensure that sensitive data is encrypted. Check config manager [docs](https://github.com/Jumpscale/core/blob/master/docs/config/configmanager.md) to get started.
