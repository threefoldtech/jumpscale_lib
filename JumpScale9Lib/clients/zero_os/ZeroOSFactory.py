...
class ZeroOSFactory:
    ...
    def zeroNodeInstall_OVH(self, OVHServerID, OVHClient, zerotierNetworkID, zerotierClient):
        ...
        while True:
            try:
                member = zt.getNetworkMemberFromIPPub(ip_pub, networkId=zerotierNetworkID, online=True)
    ...
    def zeroNodeInstall_PacketNET(self, packetnetClient, zerotierClient, project_name, plan_type, location, server_name, zerotierNetworkID, ipxe_base='https://bootstrap.gig.tech/ipxe/master'):
        ...
        valid_plan_types = ("Type 0", "Type 1", "Type 2" ,"Type 2A", "Type 3", "Type S")
        ...
        # find project id
        project_ids = [project.id for project in packetnetClient.list_projects() if project.name == project_name]
        if not project_ids:
            raise j.exceptions.NotFound('No projects found with name %s' % project_name)
        project_id = project_ids[0]

        # find service type id
        project_devices = {device.hostname: device for device in packetnetClient.list_devices(project_id)}
        if hostname not in project_devices:
            plan_ids = [plan.id for plan in packetnetClient.list_plans() if plan.name == plan_type]
            if not plan_ids:
                raise j.exceptions.NotFound('No plans found with name %s' % plan_type)
            plan_id = plan_ids[0]
            ...
            if facility_id is None:
                raise j.exceptions.Input(message="Could not find facility in packet.net:%s" % location)
            try:
                ...
                if "Service Unavailable" in str(e):
                    raise j.exceptions.Input(message="could not create packet.net machine, type of machine not available")
                raise e
        else:
        ...
        while True:
            try:
                member = zerotierClient.getNetworkMemberFromIPPub(ip_pub[0], networkId=zerotierNetworkID, online=True)
                ipaddr_priv = member["ipaddr_priv"][0]
                break
            except RuntimeError as e:
                # case where we don't find the member in zerotier
