...
def sliders():
    ...
    l = html.Div(children=[
        ...
        dcc.Slider(id='slider_marketcap_months', min=6, max=48, step=1,
                   value=16, marks={i*6: '{}'.format(i*6) for i in range(0, 9)}),
        html.Br(),
        ...
        dcc.Slider(id='exitvalue_tftech', min=0, max=2000, step=100, value=1000,
    ...
    r = html.Div(children=[
        ...
        dcc.Slider(id='nr_months_past', min=0, max=48, step=1, value=16,
                   marks={i*4: '{}'.format(i*4) for i in range(0, 12)}),
        html.Br(),
        html.Div("average contribution level for the past (2.5 is std)"),
        dcc.Slider(id='contr_level_past', min=0, max=5, step=0.5, value=2.5,
                   marks={i: '{}'.format(i) for i in range(0, 5)},),
        html.Br(),
        html.Div("average contribution level for future (2.5 is std)"),
        dcc.Slider(id='contr_level_future', min=0, max=5, step=0.5, value=2.5,
                   marks={i: '{}'.format(i) for i in range(0, 5)},),
        html.Br(),
        html.Div("% working (50% is halftime, ...)"),
        dcc.Slider(id='workingtime', min=0, max=100, step=10, value=100,
