...
Right now this class is just copied from the rivine client, so we can already
fix the method signatures without having to waste time with figuring out how to
inherit properly from the parent rivine wallent.
"""
...
class TfchainWallet:
    ...
    def __init__(self, seed, bc_networks, bc_network_password, nr_keys_per_seed=50, minerfee=100000000, client=None):
        ...
        """
        self._client = j.clients.tfchain.get(client) if client else None
    ...
    def generate_address(self):
        ...
        if self._client is not None:
            ...
            cl = j.clients.tfchain.get(instance=self._client.instance,
                                       data=data,
                                       create=True,
                                       interactive=False)
            cl.config.save()
