...
Sending money to the TFT20 network (a TFT-backed ERC20 token running on top of Ethereum) is possible as well.

```python
# Open your wallet, if not opened already.
In [1]: wallet = j.clients.tfchain.create_wallet('my_wallet')

# Send some amount of TFT to a TFT20 address,
# burning the TFT as a result while converting these into TFT20 funds.
In [2]: wallet.send_money(amount=1000, recipient='f2cf4aba72ea26fd7d91c1ec46e1c8718f891a06')
Out [2]: <JumpscaleLib.clients.blockchain.rivine.types.transaction.TransactionV208 at 0x7fc365dbad68>

# The sent TFT funds will be deducted from your wallet's balance and dissapear from the TFT network with it.
...
In[2]: j.clients.tfchain.threebot.update_record(wallet, 1,
...
In[2]: txn = j.clients.tfchain.create_transaction_from_json(txn_json)
...
Sending funds to an ERC20 address is already explained as a how-to in the [Sending Funds](#sending-funds) section.
This section will discuss any new commands introduced for the ERC20 feature.

### Register an ERC20 address

In order to register an ERC20 address on the standard network,
you need to have a wallet opened, and optionally you need to have the public key ready in string format.
Should you not have a public key ready, or do not care which public key is used, you can choose to not pass a public key,
in which case a new public key will be generated for you, using the passed wallet's seed.

```python
# Open your wallet, if not opened already.
In [1]: wallet = j.clients.tfchain.create_wallet('my_wallet')

# register an address using a newly-generated public key based upon your wallet's seed
In [2]: j.clients.tfchain.register_erc20_address(wallet)
Out[2]: <JumpscaleLib.clients.blockchain.rivine.types.transaction.TransactionV210 at 0x7fc365ca8748>
```

The same logic applies for _testnet_ as well as _devnet_,
the difference being that your wallet has to be configured for the desired network.

Should you want to register an ERC20 address for a specific address you have to specify the optional `public_key` parameter.

```python
# Open your wallet, if not opened already.
In [1]: wallet = j.clients.tfchain.create_wallet('my_wallet')

# Select or define a public key, already linked to your wallet
In [2]: public_key = wallet.public_key(wallet.addresses[0])

# register an address using a newly-generated public key based upon your wallet's seed
In [3]: j.clients.tfchain.register_erc20_address(wallet, public_key=public_key)
Out[3]: <JumpscaleLib.clients.blockchain.rivine.types.transaction.TransactionV210 at 0x7f68c6dfa358>
```

The public key has to be in byte format (`bytes`).
