...
class TransactionV144:
    ...
    def get_input_signature_hash(self, extra_objects=None):
        ...
        buffer.extend(tfbinary.BinaryEncoder.encode(self._transaction_fee, type_='currency'))
...
def sign_bot_transaction(transaction, public_key, secret_key, specifier):
    """
    Sign the pair using the secret key and fulfillment
    """
    sig_ctx = {
        'extra_objects': [specifier],
...
class FlatMoneyTransaction:
    ...
    def create_list(explorer_transaction):
        ...
        data_type = 0
        if 'arbitrarydata' in rawtxdata:
            data = base64.b64decode(rawtxdata['arbitrarydata'])
            # ad = ArbitraryData()
            # ad.value = base64.b64decode(rawtxdata['arbitrarydata'])
            # if 'arbitrarydatatype' in rawtxdata:
            #     ad.type = int(rawtxdata['arbitrarydatatype'])
