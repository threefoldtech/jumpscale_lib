...
class RivineWallet:
    ...
    def public_key(self, address):
        """
        Public key owned by this wallet and linked to the passed address.
        """
    ...
    def send_money(self, amount, recipient, data=None, locktime=None):
        ...
        if len(recipient) == 78:
            transaction = self._create_transaction(amount=amount,
                                                    ...
                                                    custom_data=data,
                                                    locktime=locktime)
        elif len(recipient) == 40 or len(recipient) == 42:
            if data is not None:
                raise ValueError("arbitrary data cannot be defined for ERC20 Transactions")
            if locktime is not None:
                raise ValueError("locktime cannot be defined for ERC20 Transactions")
    ...
    def _sign_bot_registration_transaction(self, transaction, commit=False):
        ...
        if uh not in self._keys:
            ...
            return
        key = self._keys[uh]
        transaction.identification.signature = sign_bot_transaction(transaction, transaction.identification.public_key, key.secret_key, b'sender')
        if commit:
            self._commit_transaction(transaction=transaction)
    ...
    def _sign_bot_record_update_transaction(self, transaction, commit=False):
        ...
        if uh not in self._keys:
            ...
            return
        key = self._keys[uh]
        transaction.set_signature(sign_bot_transaction(transaction, public_key, key.secret_key, b'sender'))
        if commit:
            self._commit_transaction(transaction=transaction)
    ...
    def _sign_bot_name_transfer_transaction(self, transaction, commit=False):
        ...
        if uh in self._keys:
            key = self._keys[uh]
            transaction.set_sender_signature(sign_bot_transaction(transaction, sender_pub_key, key.secret_key, b'sender'))
        ...
        if uh in self._keys:
            key = self._keys[uh]
            transaction.set_receiver_signature(sign_bot_transaction(transaction, receiver_pub_key, key.secret_key, b'receiver'))
        # commit if desired
        if commit:
            self._commit_transaction(transaction=transaction)
    ...
    def _get_public_key_from_bot_id(self, identifier):
        record = TfchainThreeBotClient.get_record(identifier, explorers=self._bc_networks)
...
class WalletBalance:
    ...
    def __str__(self):
        ...
        if len(self._locked_outputs) > 0:
            string += '\nLocked:\n'
            outputs = {}
            for (output, tl) in self._locked_outputs.values():
                outputs[tl] = outputs.get(tl, 0) + int(output['value'])
            for (tl, value) in outputs.items():
                string += '\n\t{} locked until {}\n'.format(value / HASTINGS_TFT_VALUE,
                                                        self._locktime_to_string(tl))

        if len(self._unconfirmed_locked_outputs) > 0:
            string += '\nLocked (Unconfirmed):\n'
            outputs = {}
            for (output, tl) in self._unconfirmed_locked_outputs.values():
                outputs[tl] = outputs.get(tl, 0) + int(output['value'])
            for (tl, value) in outputs.items():
                string += '\n\t{} locked until {}\n'.format(value / HASTINGS_TFT_VALUE,
                                                        self._locktime_to_string(tl))
