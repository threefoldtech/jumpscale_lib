...
class ZDBMeta(JSBASE):
    ...
    def schema_data_get(self, id=None, md5=None, url=None, die=False):
        if id is not None:
            cfg = self._data["schemas"].get(id, None)
            if cfg is None:
                if die:
    ...
    def schema_id_incr(self):
        return max(self._data['schemas'], default=0) + 1
    ...
    def schemas_load(self):
        # sort all ids
        ids = list(self._data["schemas"].keys())
        ids.sort()
        res2 = []
    ...
    def load(self):
        ...
        if data is None:
            ...
            self._data["config"] = {}
            print(self._data)
            b = j.data.serializers.msgpack.dumps(self._data)
            print('set', b)
            key = self.db.set(b)
        else:
            print('get', data)
