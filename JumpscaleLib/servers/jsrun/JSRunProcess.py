...
class JSRunProcess(JSBASE):
    ...
    def stop(self):
        ...
        if self.stopcmd:
            ...
            self.logger.warning("stopcmd:%s"%cmd)
            rc,out,err=j.sal.process.execute(cmd,die=False)
            time.sleep(0.2)
        if self.pid:
            self.logger.info("found process to stop:%s"%self.pid)
        ...
        if self.process_strings!=[]:
            for pstring in self.process_strings:
                ...
                while pids!=[]:
                    for pid in pids:
                        self.logger.debug("warning:%s"%pid)
    ...
    def wait_stopped(self,die=True,onetime=False,timeout=5):
        ...
        while onetime or j.data.time.epoch<end:
            ...
            if nr==len(self.ports):
                self.logger.info("IS HALTED %s"%self.name)
                return True
            if onetime:
                break
        if die:
        ...
        while onetime or j.data.time.epoch<end:
            ...
            for port in self.ports:
                if j.sal.nettools.tcpPortConnectionTest(ipaddr="localhost",port=port):
                    nr+=1
            if nr==len(self.ports):
                self.logger.info("IS RUNNING %s"%self.name)
                return True
            if onetime:
                break
        if die:
    ...
    def start(self,reset=False,port=None):
        ...
        if reset:
            ...
            if self.running:
                self.logger.info("no need to start was already started:%s"%self.name)
                return
