...
class Mongod:
    ...
    def start(self, timeout=30, log_to_file=False):
        ...
        if not self.is_running():
            logger.info(
                'start a member of the replica set "{}"'.format(
                    self.replica_set))
    ...
    def init_replica_set(self, hosts):
        ...
        if not self.is_running():
            raise RuntimeError(
                'Cannot initialize replica set, "{}" mongodb instance is not running'.format(
                    self.replica_set))
        ...
        self.container.client.bash(
            'echo "%s" >> %s' %
            (config, conf_file)).get()
        cmd = 'mongo --port {port} {file}'.format(
            port=self.port, file=conf_file)
...
"""
                ...
                if not mount.get(key):
                    raise ValueError(
                        """Mount point config should be set to None or given correctly.
                            ...
                            mount, """ {'storagepool': '<storagepool name>'
                                        'fs': '<fs_name>'
    ...
    def _container_data(self):
        ...
        if not candidates:
            ...
            raise RuntimeError(
                "Found multiple eligible interfaces for macvlan parent: %s" %
    ...
    def ip(self):
        ...
        while time.time() < start + timeout:
            ...
            raise RuntimeError(
                'cannot fetch container "{}" IP'.format(
                    self._container_name))
...
class Mongos:
    ...
    def start(
        ...
        while time.time() < start + timeout:
            ...
            raise RuntimeError(
                'Failed to start mongodb routing service in contaner {}'.format(
                    self.container.name))
