...
def do_pkgsandbox_and_push(prefabpkg, flistname="cryptosandbox", bins=None):
    ...
    prefab.core.execute_bash(
        'cd {} && tar -czf /tmp/{}.tar.gz .'.format(DATA_DIR, flistname))
    prefab.core.execute_bash(
...
if __name__ == "__main__":
    try:
        ...
        btcflist = do_pkgsandbox_and_push(
            prefab.blockchain.bitcoin,
            flistname="bitcoinflist",
            bins=bitcoinbins)
        tfchainflist = do_pkgsandbox_and_push(
            prefab.blockchain.tfchain,
            flistname="tfchainflist",
            bins=tfchainbins)
        ethereumflist = do_pkgsandbox_and_push(
            prefab.blockchain.ethereum,
            flistname="ethereumflist",
            bins=ethbins)
        atomicswapflist = do_pkgsandbox_and_push(
            prefab.blockchain.atomicswap,
            flistname="atomicswapflist",
            bins=atomicswapbins)
        cryptosources = [
            btcflist,
            tfchainflist,
        ...
        ubuntucryptosources = [
            ...
            ethereumflist,
            atomicswapflist,
            "gig-bootable/ubuntu:lts.flist"]

        ubuntucryptoflist = merge(
            zhub_client,
            ubuntucryptotarget,
            ubuntucryptosources)
        ...
        zhub_dataeg = {
            'token_': iyo_client.jwt_get(),
            'username': username,
            'url': 'http://192.168.20.132:8080/api'}
