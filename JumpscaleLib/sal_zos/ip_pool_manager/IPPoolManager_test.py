...
class TestIPPool(TestCase):
    ...
    def setUp(self):
        ...
        self.p = IPPool(
            id="pool_1",
            name="poolname",
            network_address="192.168.20.0/29",
            registered_ips=self._HOSTS)
        self.pool_full_range_hosts = IPPool(
            id="pool_2", name="pool2name", network_address="192.168.20.0/29")
    ...
    def test_ips_after_first_pool_creation_equal_available_ips(self):
        assert len(
            self.p.available_hosts) == len(
            self.p.available_ips) == len(
            self.p.ips) == len(
            self._HOSTS)
    ...
    def test_pool_gets_network_range_if_no_registered_ips(self):
        assert len(
            self.pool_full_range_hosts.available_ips) == len(
            self._HOSTS) + 1
    ...
    def test_len_available_ips_after_reserving_two_ips(self):
        ...
        assert len(self.p.available_ips) == len(
            self.p.registered_ips) - len(self.p.reserved_ips)
...
class TestIPPoolManager(TestCase):
    ...
    def setUp(self):
        ...
        self.p2 = IPPool(
            id="pool_2",
            name="opsnet",
            network_address="192.128.23.0/30")
    ...
    def test_all_available_ips_equals_sum_of_ips_in_registered_pools(self):
        assert len(self.mgr.available_ips) == 8 == sum(
            len(p.available_ips) for p in [self.p1, self.p2])
    ...
    def test_reserving_two_ips_decreases_available_ips_by_two(self):
        ...
        assert len(self.mgr.ips) == len(
            self.mgr.available_ips) + len(self.mgr.reserved_ips)
