...
Let's use the Docker image with JumpScale pre-installed [jumpscale/ubuntu1604_js8](https://hub.docker.com/r/jumpscale/ubuntu1604_js8/) from Docker Hub. You can of course also first [build your own Docker image](Docker_image.md).
...
def docker_create_machine(name, reinstall=False, image='despiegk/mc'):
    docker = j.atyourservice.server.findTemplates(name='node.docker')[0]
...
if __name__ == '__main__':
    ...
    instance = j.atyourservice.server.findServices(name='node.docker', instance=name)[0]

    portal = j.atyourservice.server.findTemplates(name='singlenode_portal')[0]
